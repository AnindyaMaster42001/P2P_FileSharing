...
class MainWindow:
    ...
    def on_login_success(self, result):
        ...
        # Handle the case where result is directly the port number
        if isinstance(result, int):
            port = result
        # Handle the case where result is a dictionary with a 'port' key
        elif isinstance(result, dict):
            port = result.get('port', 0)
        else:
            # Fallback if result is neither an int nor a dict
            port = 0
            logger.error(f"Unexpected result type in on_login_success: {type(result)}")
        
        # Store the user's requested port before switching screens
        requested_port = None
        if hasattr(self.current_screen, 'port_entry'):
            try:
                requested_port = int(self.current_screen.port_entry.get())
    ...
    # def show_group_invitation_notification(self, invitation):
    #     """Show notification for a group invitation"""
    #     group_name = invitation.get('group')
    #     from_user = invitation.get('from')
    ...
    #         self.app_controller.group_manager.accept_invitation(group_name, from_user)
    #         messagebox.showinfo("Joined Group", f"You have joined the group '{group_name}'")
            
    #         # If in group mode, refresh the groups list
    #         if self.app_controller.current_mode == "group":
    #             self.show_group_mode()
    #     else:
    #         self.app_controller.group_manager.decline_invitation(group_name, from_user)

    def show_group_invitation_notification(self, message):
        ...
        self.root.after_idle(
            lambda: self.app_controller.show_group_invitation_dialog(group_name, from_user)
        )
